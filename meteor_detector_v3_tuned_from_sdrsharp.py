...
# Count bins that are at least this fraction of the peak power.
# Too many => broadband burst, not narrowband ping.
NEAR_PEAK_POWER_RATIO = 0.25
MAX_NEAR_PEAK_BINS = 5

# Steady-tone rejection
STEADY_TONE_MIN_DURATION_S = 1.50
STEADY_TONE_MAX_SPREAD_HZ = 30.0

# Output
OUTPUT_DIR = "meteor_logs"
CSV_FILE = os.path.join(OUTPUT_DIR, "events_v3.csv")
SAVE_SPECTROGRAM = True
SAVE_WAV = False

SPECGRAM_MIN_HZ = 0
SPECGRAM_MAX_HZ = 3000
...
# SDR# tuning used for this profile (read from your screenshot):
# - Mode: USB
...
# - PPM correction: -66
#
# In USB mode, the detector listens to AUDIO tone, not RF directly.
# With VFO at 143.048400 MHz and the GRAVES carrier near 143.050000 MHz,
# the expected audio tone is about 1600 Hz.
# So the detection band is tightened around that audio offset.
...
def classify_event(duration_s, freq_spread_hz, max_near_peak_bins):
    if duration_s > MAX_EVENT_DURATION_S:
        return "too_long"

    if duration_s >= STEADY_TONE_MIN_DURATION_S and freq_spread_hz <= STEADY_TONE_MAX_SPREAD_HZ:
        return "steady_tone_rejected"

    if max_near_peak_bins > MAX_NEAR_PEAK_BINS:
...
def finalize_event(
    ...
    event_peak_freqs,
    event_peak_prom_values,
    event_near_peak_bins_values,
    ...
    peak_prominence_db = float(np.max(event_peak_prom_values))
    max_near_peak_bins = int(np.max(event_near_peak_bins_values))
    band_db_at_start = float(event_band_db_values[0])

    if forced_type is not None:
        event_type = forced_type
    else:
        event_type = classify_event(duration_s, freq_spread, max_near_peak_bins)

    image_file = ""
    wav_file = ""

    if SAVE_SPECTROGRAM and event_type == "meteor_candidate":
        image_file = os.path.join(OUTPUT_DIR, f"event_v3_{event_id:05d}.png")
        save_spectrogram(event_frames, SAMPLE_RATE, image_file)

    if SAVE_WAV and event_type == "meteor_candidate":
        wav_file = os.path.join(OUTPUT_DIR, f"event_v3_{event_id:05d}.wav")
    ...
    with open(CSV_FILE, "a", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            round(band_db_at_start, 2),
            round(peak_prominence_db, 2),
            max_near_peak_bins,
            event_type,
            image_file,
            wav_file,
    ...
    print(
        ...
        f"spread={freq_spread:.1f} Hz prom={peak_prominence_db:.1f} dB "
        f"near_bins={max_near_peak_bins}"
    )
...
def main():
    ...
    print(f"Peak-to-median minimum: {PEAK_TO_MEDIAN_DB_MIN} dB")
    print(f"Band-rise minimum: {BAND_RISE_DB_MIN} dB")
    ...
    with sd.InputStream(
        ...
        while True:
            ...
            near_peak_bins = int(np.sum(band_power_values >= (peak_bin_power * NEAR_PEAK_POWER_RATIO)))
            ...
            band_rise_db = band_db - baseline_db

            triggered = (
                peak_bin_db >= threshold_db
                and peak_prominence_db >= PEAK_TO_MEDIAN_DB_MIN
                and band_rise_db >= BAND_RISE_DB_MIN
                and near_peak_bins <= MAX_NEAR_PEAK_BINS
            )
            ...
            if in_event:
                ...
                event_peak_prom_values.append(peak_prominence_db)
                event_near_peak_bins_values.append(near_peak_bins)
                event_band_db_values.append(band_db)
                ...
                if (now_ts - event_start_ts) >= MAX_EVENT_DURATION_S:
                    ...
                    finalize_event(
                        ...
                        event_peak_prom_values=event_peak_prom_values,
                        event_near_peak_bins_values=event_near_peak_bins_values,
                        event_band_db_values=event_band_db_values,
